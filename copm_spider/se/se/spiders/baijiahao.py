...
import codecs
import re
from se.items import BaijiahaoItem
import pytz
...
class BaijiahaoSpider(scrapy.Spider):
    ...
    @classmethod
    def _search_query(cls, q1=[], q2=[], q3=[], q4=[], q5='', q6='', ft='', gpc=''):
        """
        百度搜索的规则如下：
        q1: 包含以下全部的关键词
        q2: 包含以下的完整关键词
    ...
    def start_requests(self):
        ...
        for key in self.keyword:
            url = BaijiahaoSpider._search_query(q2=[key], q6='baijiahao.baidu.com')
    ...
    def parse(self, response):
        ...
        titles = BaijiahaoSpider._extract_title(titles)
        titles = [
            BaijiahaoSpider._filter_tags(title)
            for title in titles
        ]
        dates = BaijiahaoSpider._extract_date(dates)
        abstracts = BaijiahaoSpider._extract_abstract(abstracts)
        abstracts = [
            BaijiahaoSpider._filter_tags(abstract)
            for abstract in abstracts
        ]
