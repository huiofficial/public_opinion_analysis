...
class SePipeline(object):
    ...
    def process_item(self, item, spider):
        ...
        sql = '''INSERT IGNORE INTO content (address, title, date, abstract, keyword, mediaName, emotion, uniqueCode, writeDate, city) VALUES('{address}', '{title}',
                '{datetime}', '{abstract}', '{keyword}', '{mediaName}', {emotion}, '{uniqueCode}', '{writeDate}', '{city}')'''.format(address=item['url'], title=item['title'], datetime=item['date'], abstract=item['abstract'], 
                keyword=item['keyword'], mediaName=item['mediaName'], emotion=emotion, uniqueCode=uniqueCode, writeDate=writeDate, city=self.city_code[city])
        self.cursor.execute(sql)
        self.conn.commit()
        return item
