...
class ToutiaoPipeline(object):
    ...
    def process_item(self, item, spider):
        city = re.search(self.city_pattern, item['title'] + ' ' + item['content'])
        if city is None:
            city = '不限'
        else:
            city = city.group()
        ...
        if 'items' not in result.keys():
            ...
            if emotion == 0:
                ...
                emotion = 1
        md5 = hashlib.md5()
        uniqueStr = 'toutiao-{}'.format(item['url'])
        ...
        sql = '''INSERT IGNORE INTO content (address, title, date, abstract, keyword, mediaName, emotion, uniqueCode, writeDate, city) VALUES('{address}','{title}',
                '{date}', '{abstract}', '{keyword}', '{mediaName}', {emotion}, '{uniqueCode}', '{writeDate}', '{city}')'''.format(
            address=item['url'], title=item['title'][:20],
            date=item['date'], abstract=item['abstract'], keyword=item['keyword'],
            mediaName=item['mediaName'], emotion=emotion, uniqueCode=uniqueCode, writeDate=writeDate, city=self.city_code[city])
        self.cursor.execute(sql)
        self.conn.commit()
        return item